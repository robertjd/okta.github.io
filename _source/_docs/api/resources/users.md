...
> When fetching a user by `login` or `login shortname`, you should [URL encode](http://en.wikipedia.org/wiki/Percent-encoding) the request parameter to ensure special characters are escaped properly.  Logins with a `/` and '?'  character can only be fetched by `id` due to URL issues with escaping the `/` and '?' characters.
