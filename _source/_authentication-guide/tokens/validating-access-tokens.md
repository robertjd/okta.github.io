...
As mentioned above, it is important that the resource server (your server-side application) only accept the access token from a client. This is because access tokens are intended for authorizing access to a resource, which is exactly the use case you have here.
[//]: # (what case do i have here?  also: makes me think that the token can come from something other than a client)
...
[//]: # (might want to defer the caching conversation until later)
...
[//]: #(do we have some other docs that dive into this more, e.g. “it’s base 64 url encoded, period separated, blah blah” ?)
