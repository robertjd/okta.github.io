 ...
 */
function loadContent(client, server, framework) {
 ...
 *
 * @returns [client, server, framework]
 */
function getSelectionTupleFromHash() {
  var matches = window.location.hash.match(urlExpression);
  var client = matches && matches[1];
  var server = matches && matches[2];
  var framework = matches && matches[3];
  return [client, server, framework];
}
